...
for $twitter_username in $usernames {
        ...
        post $user_endpoint -t application/x-www-form-urlencoded [ variables ($variables|to json -r) ] -H [ Authorization $bearer,  x-guest-token $token ] | get data.user.result | select legacy.name legacy.screen_name legacy.description legacy.protected legacy.verified legacy.created_at legacy.followers_count rest_id has_nft_avatar
}
