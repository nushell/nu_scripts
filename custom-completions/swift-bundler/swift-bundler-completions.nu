def "nu-complete sbun bundle-arg-values" [arg_name: string]: nothing -> list<string> {
...
def "nu-complete sbun bundler" [] {
    nu-complete sbun bundle-arg-values bundler
}
def "nu-complete sbun configuration" [] {
    nu-complete sbun bundle-arg-values configuration
}
def "nu-complete sbun arch" [] {
    nu-complete sbun bundle-arg-values arch
}
def "nu-complete sbun platform" [] {
    nu-complete sbun bundle-arg-values platform
}
def "nu-complete sbun apps" [context: string] {
    ...
    swift-bundler config apps list --json --directory $directory | from json | get name
}
def "nu-complete sbun templates" [] {
    swift-bundler templates list | lines | parse "* {value}: {description}"
}
def "nu-complete sbun devices" [] {
    {
        ...
        completions: (swift-bundler devices list
            ...
            | each {
                if $in.uuid != null {
                    rename --column {uuid: "value", device: "description"}
                } else {
                    rename --column {device: "value"}
                }
            }
            | prepend "host")
    }
...
def "nu-complete sbun simulators" [] {
    {
        ...
        completions: (swift-bundler simulators list
            ...
            | each {
                if $in.uuid != null {
                    rename --column {uuid: "value", simulator: "description"}
                } else {
                    rename --column {simulator: "value"}
                }
            })
    }
}
...
export extern "swift-bundler bundle" [
    ...
    # The directory containing the package to build. This changes the working directory before any other operation.
    --config-file: path
    ...
    --arch(-a): string@"nu-complete sbun arch"
    # The architectures to build for. Defaults to the host architecture.
    --scratch-path: directory
    # A custom scratch directory path. Defaults to `.build` in the current directory.
    --Xswiftpm: string
    ...
    --platform(-p): string@"nu-complete sbun platform"
    # The platform to build for. Defaults to the host platform.
    --device: string@"nu-complete sbun devices"
    # A device name, id or search term to select a target device (e.g. `"Apple TV"` or `"John Appleseed's iPhone"`). Can be a simulator. Use `"host"` to refer to the host machine.
    --strip
    ...
    # The provisioning profile to embed in the app (only applicable when targeting non-macOS physical Apple devices).
    --codesign
    ...
    --xcodebuild
    # Build with xcodebuild instead of SwiftPM. This is the default when building for non-macOS Apple platforms from a Mac, since SwiftPM has issues doing so.
    --no-xcodebuild
    # Force swiftpm to be used when targeting non-macOS Apple platforms. Use with care because many features, such as conditional dependencies in package manifests, may break.
    --skip-build
    # Skip the build step.
    --show-bundle-path
    # Print the path of the output bundle and exits. The bundle may not exist yet if you haven't already performed a build.
    --built-with-xcode
    # Treats the products in the products directory as if they were built by Xcode (which is the same as universal builds by SwiftPM). Can only be set when `--skip-build` is supplied.
    --verbose(-v)
...
export extern "swift-bundler run" [
    app_name?: string@"nu-complete sbun apps"
    # The name of the app to build.
...
export extern "swift-bundler clean" [
    --directory(-d): directory
...
export extern "swift-bundler create" [
    app_name: string
    ...
    # An alternative Swift toolchain to use.
    --indentation: string
    # Custom indentation style: either `"tabs"` or `"spaces=[count]"`. Defaults to `"spaces=4"`.
    --force(-f)
...
export extern "swift-bundler convert" [
    xcode_file: path
...
export extern "swift-bundler config apps" [
    --directory(-d): directory
...
# List apps in the current Swift Bundler project.
export extern "swift-bundler config apps list" [
    --directory(-d): directory
    # The directory containing the package to inspect.
    --json
    # Format command output as JSON.
...
export extern "swift-bundler config migrate" [
    --directory(-d): directory
...
export extern "swift-bundler devices" [
    --verbose(-v)
    # Print verbose error messages.
    --version
    # Show the version.
...
# List available iOS, tvOS, visionOS, and Android devices.
export extern "swift-bundler devices list" [
    --verbose(-v)
    # Print verbose error messages.
    --version
    # Show the version.
...
export extern "swift-bundler simulators" [
    filter?: string
...
# List available iOS, tvOS visionOS, and Android simulators.
export extern "swift-bundler simulators list" [
    filter?: string
    # A search term to filter simulators with.
    --verbose(-v)
    # Print verbose error messages.
...
export extern "swift-bundler simulators boot" [
    id_or_name: string@"nu-complete sbun simulators"
    # The id or name of the simulator to start. Substring matching is used when filtering simulator names.
    --verbose(-v)
    # Print verbose error messages.
    --version
    # Show the version.
...
export extern "swift-bundler simulators simctl boot" [
    id_or_name: string@"nu-complete sbun simulators"
    # The id or name of the simulator to start. Substring matching is used when filtering simulator names.
...
export extern "swift-bundler simulators avd boot" [
...
export extern "swift-bundler templates" [
    --template-repository: directory
...
# List available templates.
export extern "swift-bundler templates list" [
    --template-repository: directory
    # An alternate directory to search for templates in.
    --verbose(-v)
    # Print verbose error messages.
...
export extern "swift-bundler templates info" [
...
export extern "swift-bundler templates update" [
    --verbose(-v)
    # Print verbose error messages.
    --version
    # Show the version.
...
export extern "swift-bundler generate-xcode-support" [
    --directory(-d): directory
...
export extern "swift-bundler list-identities" [
    --verbose(-v)
    # Print verbose error messages.
    --version
    # Show the version.
