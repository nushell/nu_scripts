...
--yadm-bootstrap # Override the location of the yadm bootstrap program
  ...
  bash_yadm ...(generate_viable_bash_string_flags {
    yadm-dir: $yadm_dir,
    yadm-data: $yadm_data,
    yadm-repo: $yadm_repo,
...
def generate_viable_bash_string_flags [
  ...
  # Since bash can only accept strings and string-like values the user must type them.
       ...
       # If flag is a non-bool primitive create --key=value flag.  
             if $type in $viable_types { 
     
               return $"--($key)=($value)"
             } 
       # If flag is a nothing return null 
      
             if $value == false  { return }
       
        # If flag is a bool primitive create --key flag. 
...
def "nu-complete yadm files" [] {
  ...
  bash_yadm status -uall --porcelain=2
  | lines
  | each { |$it|
    if $it starts-with "1 " {
...
export def branch [
  ...
  --track(-t)                                                    # when creating a branch, set upstream
     ...
     bash_yadm branch ...(generate_viable_bash_string_flags {
          abbrev:$abbrev,
...
export def tag [
  --delete(-d): string@"nu-complete yadm tags"         # delete a tag
     ...
     bash_yadm tag ...(generate_viable_bash_string_flags, {delete: $delete})
}
...
export def clone [
  ...
  ...args
     ...
     bash_yadm clone ...(generate_viable_bash_string_flags {
       help: $help
       local: $local
       no-local: $no_local
...
export def grep [
  ...
  bash_yadm grep ...(generate_viable_bash_string_flags {
